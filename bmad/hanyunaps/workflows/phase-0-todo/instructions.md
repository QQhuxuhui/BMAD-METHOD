# Hanyun APS Phase-0 Todo – Workflow Instructions
...
**IMPORTANT**: Introduce the **expert team** to the user first and output it to template slot `expert_team_intro` (use canonical team names below):
...
- A numbered Todo list with clear steps, **owners (specific expert names from the team above, e.g., 系统编排协调智能体/领域应用专家智能体/约束模式专家智能体/目标优化专家智能体/调度算法专家智能体/质量与评测智能体)**, and expected outputs
- HITL checkpoints with triggers:
  - P0: Todo mandatory confirmations (标注与哪位专家交互)
  - P1: confidence &lt; 0.70
...
**Todo list format** should indicate which expert owns each task:

```
[ ] Task N: {Task description}
    负责专家: 🏭/@领域应用专家智能体  （仅可填写上方团队中的标准名称之一）
    预计时间: Xmin
    交互点: 🔒 P0 需确认场景识别结果
```
