"""Constraint Expert Agent Node for BMAD Workflow.

The Constraint Expert is part of Phase 1 (P1), responsible for:
- Identifying all constraints (hard and soft)
- Classifying and prioritizing constraints
...
PROMPT_FILE = Path(__file__).parent.parent / "prompts" / "constraint.md"
with open(PROMPT_FILE, 'r', encoding='utf-8') as f:
    CONSTRAINT_PROMPT = f.read()
...
async def constraint_expert_node(state: WorkflowState) -> Dict[str, Any]:
    """Constraint Expert agent node - Phase 1 (P1).

    Identifies and classifies constraints for the optimization problem.
    ...
    """
    logger.info(
        "constraint_expert_started",
        thread_id=state.get("thread_id"),
        has_algorithm_output=bool(state.get("algorithm_output"))
    )
...
Please identify and classify all constraints in JSON format.""")
        ])
        ...
        logger.info("constraint_expert_calling_llm", thread_id=state.get("thread_id"))

        response = await _call_llm_with_retry(chain, input_data)

        # Parse JSON response
        try:
            constraint_output = json.loads(response.content)
        except json.JSONDecodeError as e:
            logger.error(
                "constraint_expert_json_parse_failed",
                error=str(e),
        ...
        logger.info(
            "constraint_expert_completed",
            thread_id=state.get("thread_id"),
            has_constraints=bool(constraint_output.get("constraint_categories")),
            tokens=tokens,
            cost=cost
        )
        ...
        return {
            ...
            "messages": [
                SystemMessage(content=CONSTRAINT_PROMPT[:200] + "..."),
        ...
        errors.append(f"Constraint Expert failed: {str(e)}")
